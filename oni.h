...
//! @brief  Функция-нормализатор текста, записанного в буфер
//!
//! @param  buffer  Указатель на первую ячейку буфера, в котором хранится текст
//!
//! @return Размер преобразованного функцией буфера в байтах
//!
//! @note   Функция удаляет из текста идущие подряд "пустые" символы (табуляция, пробел и т.д. - кроме перехода на новую строку), заменяя их на одиночные, удаляет "пустые" символы в начале и в конце каждой строки
//! @note   Функция исполняется "за n"
...
//!
//! @warning    Для корректной работы этой функции необходимо, чтобы КАЖДАЯ строка текста (включая последнюю) завершалась переходом на следующую строку 
